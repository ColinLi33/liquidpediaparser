...
pageURL = "https://liquipedia.net/heroes/Evil_Geniuses"
...
def getRosterDivs(soup):
    ...
    tableDiv = soup.select_one("div.table-responsive:nth-child(13)")
    table = tableDiv.find("table")
    tbody = table.find("tbody")
...
def getRosterHistory(rosterRows):
    ...
    with open('./hots/roster_history.csv', 'w', newline='', encoding='utf-8') as file:
...
def getFirstPlaces(tableRows):
    ...
    with open('./hots/first_places.csv', 'w', newline='') as file:
        writer = csv.writer(file)
...
def getSTierEvents(tableRows):
    ...
    with open('./hots/s_tier_events.csv', 'w', newline='') as file:
        writer = csv.writer(file)
