...
pageURL = "https://liquipedia.net/leagueoflegends/Evil_Geniuses"
...
def getRosterDivs(soup):
    outerDiv = soup.select_one("div.tabs-dynamic:nth-child(13)")
...
def getRosterHistory(rosterDivs):
    ...
    with open('./lol/roster_history.csv', 'w', newline='', encoding='utf-8') as file:
...
def getFirstPlaces(tableRows):
    ...
    with open('./lol/first_places.csv', 'w', newline='') as file:
        writer = csv.writer(file)
...
def getSTierEvents(tableRows):
    ...
    with open('./lol/s_tier_events.csv', 'w', newline='') as file:
        writer = csv.writer(file)
