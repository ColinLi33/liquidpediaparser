...
pageURL = "https://liquipedia.net/pubg/Evil_Geniuses"
...
def getRosterHistory(rosterRows):
    ...
    with open('./pubg/roster_history.csv', 'w', newline='', encoding='utf-8') as file:
...
def getFirstPlaces(tableRows):
    ...
    with open('./pubg/first_places.csv', 'w', newline='') as file:
        writer = csv.writer(file)
...
def getSTierEvents(tableRows):
    ...
    with open('./pubg/s_tier_events.csv', 'w', newline='') as file:
        writer = csv.writer(file)
