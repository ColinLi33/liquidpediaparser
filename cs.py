...
pageURL = "https://liquipedia.net/counterstrike/Evil_Geniuses"
...
def getRosterHistory(rosterDivs):
    ...
    with open('./cs/roster_history.csv', 'w', newline='', encoding='utf-8') as file:
...
def getSTierEvents(tableRows):
    ...
    for row in tableRows:
        ...
        if sTierCell is not None:
            ...
            if placementCell['data-sort-value'] in ["A1", "A2", "A3"]:
                tournamentName = cells[6].text
                winnings = cells[-1].text
    ...
    with open('./cs/s_tier_events.csv', 'w', newline='') as file:
        writer = csv.writer(file)
